...
## 17. Design Guardrails（設計防線，v1 必守）

本章節用於防止 Reference Agent 在實作或需求擴充過程中，被誤解或擴張為通用 Agent / Workflow 平台。

### G1. Router 不得進行 dynamic re-plan
- Router 僅能在執行前選擇策略
- 不得根據查詢結果品質重新選擇或調整策略
- 不得形成「查詢 → 評估 → 再查詢」循環

### G2. Strategy 不可串接
- Strategy instance 為不可再組合的最小執行單位
- Reference Agent 不提供 strategy chaining 或 workflow 定義能力
- 多步行為僅存在於單一 strategy instance 的內部

### G3. Evidence 不得觸發新查詢
- Evidence 不足只影響回應狀態（SUCCESS / PARTIAL / EMPTY）與模板
- v1 不支援自動補救、隱性 retry 或品質驅動的再次查詢

### G4. TOOLS.md 非 Tool Registry
- TOOLS.md 為靜態能力宣告檔
- 不支援動態工具探索、推薦或自動選用
- 工具選擇僅依賴 Profile allowlist 與 Router 規則

### G5. 非平台級多租戶
- v1 預設為 per-tenant instance 部署
- 不提供 RBAC、配額、帳務等 SaaS 控制平面能力

### G6. LLM 不得主導策略選擇
- LLM 僅用於意圖分類、受限欄位抽取與 External 合併答案生成
- 所有策略選擇必須可由規則與 rationale codes 解釋
