...
class AnswerComposer:
    ...
    def compose_multi(self, query: str, tool_answers: List[tuple[str, str]], evidence: List[Evidence]) -> str:
        if not self._llm or not self._model:
            return self._fallback_multi(tool_answers)
        prompt = self._build_multi_prompt(query, tool_answers, evidence)
        return self._llm.generate(self._model, LLMRequest(prompt, 0.2, 768)).strip() or self._fallback_multi(
            tool_answers
        )
    ...
    def _fallback_multi(self, tool_answers: List[tuple[str, str]]) -> str:
        if not tool_answers:
            return ""
        parts = []
    ...
    def _build_multi_prompt(
        self, query: str, tool_answers: List[tuple[str, str]], evidence: List[Evidence]
    ) -> str:
        citations = "\n".join(f"- {item.tool_id}: {item.locator.model_dump()}" for item in evidence)
        sources = "\n\n".join(f"[{tool_id}]\n{answer}" for tool_id, answer in tool_answers)
        return (
            "Task:\n"
            "You are an answer generator within a RAG system that helps to compose an proper answer in response to "
            f"the question or instruction \"{query}\" from the user. Produce helpful, accurate, and "
            ...
            "the information provided below:\n\n"
            "<INFORMATION>\n"
            "Retrieved data from multiple tools\n"
            f"{sources}\n\n"
            "Evidence locators\n"
            f"{citations}\n"
            "<INFORMATION>\n\n"
