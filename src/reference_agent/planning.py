...
class PlanSkeletonBuilder:
    ...
    def _prompt(
        ...
        return (
            ...
            "Definitions:\n"
            "- answer_blueprint: the key points or sub-questions the final answer must cover. "
            "This is NOT a task decomposition.\n"
            "- required_fields: structured fields or identifiers that must appear in the answer "
            "(e.g., cve_id, entity_id, time_range, doc_id). These are for coverage checks, not prose topics.\n\n"
            f"{allowed_note}"
            f"Query: {query}\n\n"
